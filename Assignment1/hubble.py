...
# Groups
g = [ [ 0, 1 ], [ 3 ], [ 4, 5, 6 ], [ 13, 20, 23, 22, 21, 12 ],
      [ 10, 19, 7, 9, 18, 16, 8, 6 ], [ 11 ], [ 14 ], [ 15 ], [ 17 ] ]
...
for group in g:
   nogm = len(group)
   rg.append(math.fsum([r[i] for i in group])/nogm)
   vg.append(math.fsum([v[i] for i in group])/nogm)

n = len(rg)   # number of galaxies
...
if n > 2 : 
      sigma = math.sqrt(sum((vg[i] - (a+b*rg[i]))**2 for i in range(n)) / (n-2))
