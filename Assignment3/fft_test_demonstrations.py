...
from fft import fft, ifft, fft_power
from numpy import array, real, polyfit, poly1d
import math
import time
...
#                yinput[iy] = yinput[iy] * (0.5 - 0.5 * math.cos(2*math.pi*iy/float(N-1)))
    ...
    pfit = polyfit(x[:len(xinput)], y[:len(xinput)], 1)
    pfun = poly1d(pfit)
    pfy = pfun(x)
    ...
    yls = array( [y[i]-pfy[i] for i in range(len(y))] )

    Yls = fft(yls) 

    maxfreq = 50
    # Now smooth the data
    for iY in range(maxfreq, len(Yls)-maxfreq ) :
        Yls[iY] = complex(0,0)

    poweryls = fft_power(Yls)

    Yre = [math.sqrt(Y[i].real**2+Y[i].imag**2) for i in xrange(len(Y))]

    ysmoothed = ifft(Yls)
    ysmoothedreal = real(ysmoothed)
    ysmoothedreal = [ysmoothedreal[i]+pfy[i] for i in range(len(ysmoothedreal))]
    ...
    p1, = plt.plot( powerx, powery )
    p2, = plt.plot( powerx, poweryls )
    ax.legend( [p1, p2], ["Power - Data", "Power - Smoothed"], loc=4 )
    plt.yscale('log')
