...
#                yinput[iy] = yinput[iy] * (0.5 - 0.5 * math.cos(2*math.pi*iy/float(N-1)))
    ...
    pfit = lambda c: [c[0]*(xx**2) + c[1]*xx + c[2] for xx in xinput]
    chi2 = lambda xi: sum([((e-o)**2)/abs(e) for e, o in zip(xi, yinput)])

    x0 = (0., 0.1, 300.)

    pfc = opt.minimize(lambda c: chi2(pfit(c)), x0, method='Nelder-Mead')

    print "quadratic fit: "
    print pfc

    data_file_name = 'temp.txt'
    file = open(data_file_name, 'r')
    lines = file.readlines()
    file.close()
    print ' read', len(lines), 'lines from', data_file_name
    ...
    plt.subplot(2, 1, 1)
    plt.plot( x, y, '-', x, pfit(pfc['x']), "--" )

    ax = plt.subplot(2, 1, 2)
    plt.plot( [(t-1958)*12 for t in yr],temps,'-' )
